package com.track.login.authentication;
...
public class MyAuthenticationFilter extends BasicAuthenticationFilter {
    ...
    public MyAuthenticationFilter(AuthenticationManager authenticationManager, Boolean tokenRedis, Integer tokenExpireTime,
                                  Boolean storePerms, StringRedisTemplate redisTemplate, SecurityUtils securityUtils) {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        MDC.put(Constants.USER_ID, String.valueOf(securityUtils.getSysCurrUser().getId()));
        MDC.put(Constants.USER_NAME, securityUtils.getSysCurrUser().getUsername());
        try {
            chain.doFilter(request, response);
        } finally {
            if (bInsertMDC) {
    ...
    private UsernamePasswordAuthenticationToken getAuthentication(String header, HttpServletResponse response) {
        ...
        if (tokenRedis){
            ...
            //该token存在
                ...
                //判断用户是否存在
                securityUtils.userIsExit(username);
                ...
                if (loginType.equals(LoginTypeEnum.MANAGE_PASSWORD.name()) || loginType.equals(LoginTypeEnum.MANAGE_CODE.name())) {
                    ...
                    //不缓存权限,需要读取权限(需要从数据库获取)
                        ...
                        authorities = securityUtils.getSysCurrUserPerms(username);
                    }
                }
